...
func (c *CommandArgs) Completion() {
	envr := os.Environ()
	envName := make(predict.Set, 0, len(envr))
	for _, v := range envr {
		envName = append(envName, strings.Split(v, "=")[0])
	}
	//cmd := complete.FlagSet(flag.CommandLine)
	...
	complete.Complete(flag.CommandLine.Name(), cmd)
}
